...
//    Page<ProductQuery> findByDeletedIsNull(Pageable pageable);
 ...
 // Soft delete a product by ID if product against id exists.
    @Modifying
    @Transactional
    @Query("UPDATE Product p SET p.deleted = true, p.updatedAt = CURRENT_TIMESTAMP WHERE p.id = :id AND p.deleted = false")
